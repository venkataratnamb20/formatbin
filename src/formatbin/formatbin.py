...
def get_transformed_data(filename: os.PathLike, n: int) -> dict:
    ...
    write_list_to_file(
        data_xor, filename.parent / filename.stem / f"{filename.stem}_c.txt"
    )
    transformed_data_dict = {i: data[i : len(data) : n] for i in range(0, n)}
    return transformed_data_dict


def get_sum_of_elements_of_dict(data_dict, idx):
    return sum([data_dict[k][idx] for k in data_dict.keys()])


def process_binary_data(
    filename: os.PathLike, n: int, output_filenames_list: list = []
) -> None:
    """Create a dictionary containing the first n lines of a binary file.
    Read data and create n chunks of data.
    arguments:
    ...
    data_xor_dict = {k: get_xor_of_list(v) for k, v in transformed_data_dict.items()}

    # sum- data_xor_dict[k][i] for k = 0, n-1, sum of values of dict for a given index
    data_xor_sum_list = [
        get_sum_of_elements_of_dict(data_xor_dict, idx)
        for idx in range(len(list(transformed_data_dict.values())[0]))
    ]
    ...
    _default_output_files_list = [
        output_data_path / f"{filename.stem}_{'0' * (k <= 9)}{k}.txt" for k in range(n)
    ]
    output_filenames_list = (
        output_filenames_list if output_filenames_list else _default_output_files_list
    )
    ...
    write_list_to_file(
        data_xor_sum_list, filename=output_data_path / f"{filename.stem}_a.txt"
    )
    write_list_to_file(
        transformed_data_xor_list, filename=output_data_path / f"{filename.stem}_b.txt"
    )
...
def main():
    ...
    parser.add_argument("-f", "--filename", type=Path, help="Path to the input file")
    parser.add_argument(
        "-n", "--nbit", type=int, default=8, help="Number of chunks to create"
    )
    parser.add_argument(
        "-o",
        "--outfilenames",
